...
// SPDX-License-Identifier: MIT
contract Wallet is Administered {
    using SafeMath for uint256;

    event Deposit(address indexed from, uint256 indexed value);
    event TransferETH(address indexed to, uint256 indexed amount);

    constructor() public Administered(msg.sender) {
        addUser(msg.sender);
    }
    ...
    function batchTransferETH(address payable[] memory _contributors, uint256[] memory _amount) public payable onlyUser {
        uint8 i = 0;
