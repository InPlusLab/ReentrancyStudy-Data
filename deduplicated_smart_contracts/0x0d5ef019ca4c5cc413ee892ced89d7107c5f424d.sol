...
contract WorkerPool is OwnableOZ, IexecHubAccessor, MarketplaceAccessor
{
  ...
  event FinalizeWork            (address indexed woid, string stdout, string stderr, string uri);
	...
	public
	{
		...
		setImmutableOwnership(tx.origin); // owner ¡ú tx.origin
...
contract IexecHub
{
	...
	*/
	...
	internal returns (uint256)
	{
		...
		// DATASET
		if (_dataset != address(0)) // address(0) ¡ú no dataset
		{
	...
	public onlyRegisteredWoid(_woid) returns (bool)
	{
		...
		// WORKERPOOL ¡ú rewarding done by the caller itself
		...
		// Rien ne se perd, rien ne se cr¨¦e, tout se transfere
