...
import './IHiSwapERC20.sol';
import './SafeMath.sol';

contract HiSwapERC20 is IHiSwapERC20 {
    using SafeMath for uint;

    string public constant name = 'HiSwap Token';
    string public constant symbol = 'HI-V1';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'HiSwap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'HiSwap: INVALID_SIGNATURE');
