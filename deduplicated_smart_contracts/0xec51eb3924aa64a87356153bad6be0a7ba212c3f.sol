...
contract LYCCoin is IERC20, Ownable {
    ...
    function burnFrom(address _from, uint256 _value) onlyOwner public returns (bool success) {
        ...
        allowed[_from][msg.sender] =  SafeMath.sub( allowed[_from][msg.sender],  _value );             // Subtract from th sender's allowance
	...
	function freezeFrom(address _from,  uint256 _value) onlyOwner public returns (bool success) {
        ...
        balances[ _from ] = SafeMath.sub(balances[ _from ], _value);                      // Subtract from the sender
        freezeOf[ _from ] = SafeMath.add(freezeOf[ _from ], _value);                        // Updates totalSupply
        emit Freeze( _from , _value);
        return true;
    }
	
	function unfreezeFrom(address _from, uint256 _value) onlyOwner public returns (bool success) {
        if (freezeOf[ _from ] < _value) revert();            // Check if the sender has enough
		if (_value <= 0) revert(); 

        freezeOf[ _from ] = SafeMath.sub(freezeOf[ _from ], _value);                      // Subtract from the sender
