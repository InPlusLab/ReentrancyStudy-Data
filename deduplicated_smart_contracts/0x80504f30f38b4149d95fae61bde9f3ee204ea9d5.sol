...
contract Daon is
    ...
    Freezable
{
    string constant private _name = "Daon";
    string constant private _symbol = "DEB";
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "DEB/transfer : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "DEB/transferFrom : Should not send to zero address"
        );
        ...
        _approve(
            ...
            _allowances[from][msg.sender].sub(
                amount,
                "DEB/transferFrom : Cannot send more than allowance"
            )
        );
        success = true;
    }
        ...
        returns (bool success)
    {
        require(
            spender != address(0),
            "DEB/approve : Should not approve zero address"
        );
