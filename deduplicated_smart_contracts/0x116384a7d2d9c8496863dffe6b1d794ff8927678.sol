    ...
    function canTransferBalance(address addr) public view returns (uint256){
        if(initTypes[addr]==0x0){
            ...
            if(initTypes[addr]==0x1){
                if(s >= 2190 minutes){
                    return balanceOf[addr];    
                }else if(s >= 1095 minutes){
                    return balanceOf[addr].add(frozenBalances[addr].mul((s.sub(1095 minutes).div(1 minutes) + 1)).div(1095)).sub(frozenBalances[addr]);
                }else{
                ...
                if(s >= 1460 minutes){
                    return balanceOf[addr];    
                }else if(s >= 730 minutes){
                    return balanceOf[addr].add(frozenBalances[addr].mul((s.sub(730 minutes).div(1 minutes) + 1)).div(730)).sub(frozenBalances[addr]);
                }else{
                    return balanceOf[addr].sub(frozenBalances[addr]);
                }
            }else{
