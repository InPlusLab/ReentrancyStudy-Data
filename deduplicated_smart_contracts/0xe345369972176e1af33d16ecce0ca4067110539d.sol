...
// File: contracts/VaultsLite.sol

// Vaults Lite distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract VaultsLite is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SOCKETs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accSocketPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws tokens to a pool. Here's what happens:
        //   1. The pool's `accSocketPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SOCKETs to distribute per block.
        uint256 accSocketPerShare; // Accumulated SOCKETs per share, times 1e12. See below.
    ...
    // Update the given pool's SOCKETs allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending SOCKETs on frontend.
    function pendingSocket(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accSocketPerShare = pool.accSocketPerShare;

        return user.amount.mul(accSocketPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the SOCKET token.
    // ----
    uint256 private socketBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = socket.balanceOf(address(this)).sub(socketBalance);
        
        if(newRewards > 0) {
            socketBalance = socket.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 socketRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        socketRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 socketRewardFee = socketRewardWhole.mul(DEV_FEE).div(10000);
        uint256 socketRewardToDistribute = socketRewardWhole.sub(socketRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(socketRewardFee);

        pool.accSocketPerShare = pool.accSocketPerShare.add(
            socketRewardToDistribute.mul(1e12).div(tokenSupply)
        );
    }

    // Deposit  tokens to VaultsLite for SOCKET allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accSocketPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accSocketPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw tokens from VaultsLite.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid, from); // Update balances of from this is not withdrawal but claiming SOCKET farmed
    ...
    function getReward(uint256 _pid) public {
        ...
        // Transfer pending tokens
        // to user
        uint256 reward = updateAndPayOutPending(_pid, msg.sender);

        user.rewardDebt = user.amount.mul(pool.accSocketPerShare).div(1e12);
        emit RewardPaid(msg.sender, _pid, reward);
    }

    function updateAndPayOutPending(uint256 _pid, address from) internal returns (uint256) {
        UserInfo storage user = userInfo[_pid][from];
        ...
        uint256 pending = user
            .amount
            .mul(pool.accSocketPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeSocketTransfer(from, pending);
        }
    ...
    // Safe socket transfer function, just in case if rounding error causes pool to not have enough SOCKETs.
    function safeSocketTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return; 

        uint256 socketBal = socket.balanceOf(address(this));
        if (_amount > socketBal) {
            socket.transfer(_to, socketBal);
            socketBalance = socket.balanceOf(address(this));
        } else {
            socket.transfer(_to, _amount);
            socketBalance = socket.balanceOf(address(this));
        }
    ...
    function transferDevFee() public {
        ...
        uint256 socketBal = socket.balanceOf(address(this));
        if (pending_DEV_rewards > socketBal) {
            socket.transfer(devaddr, socketBal);
            socketBalance = socket.balanceOf(address(this));
        } else {
            socket.transfer(devaddr, pending_DEV_rewards);
            socketBalance = socket.balanceOf(address(this));
        }
    ...
    // allowing SOCKET governance token holders to do this functions.
