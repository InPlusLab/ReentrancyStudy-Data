...
contract EnchantedShop is Shop {
    ...
    */
    ...
    address public destinationAddress = 0xc9067445962B2f613e5A300386f7c4628f604577;

    function claimFunds() onlyContributors public {
        // must use onlyContributors (itemsOwned > 0) as a check here!
        
        uint latest = latestBalanceCheck[destinationAddress];
        uint supply = object.totalSupply();
        uint balance = balanceOwed[destinationAddress];
        uint earnings = itemsOwned[destinationAddress].div(1 ether).mul(supply.sub(latest)).div(1 ether).mul(itemReturn);
        
        uint excess = excessEth[destinationAddress];
        ...
        latestBalanceCheck[destinationAddress] = supply;
        balanceOwed[destinationAddress] = 0;
        excessEth[destinationAddress] = 0;
